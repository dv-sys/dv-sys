...
const Cart = () => {
    ...
    return (
        <>  
            <div style={{width:'100%'}}>
                <div style={{float:'left',width:'60%'}}><h1>My Cart</h1></div>
                <div style={{float:'left',textAlign:'right',width:'30%',padding:'8px'}}><h2>Total Item : {product.length > 0 ? product.length : 0}</h2></div>
            </div>
            <br/><br/> 
            <hr/>
            <div style={{marginLeft:'10px'}}>
                <Button className='btn btn-warning'><Link to='/'>Back to Main</Link></Button>
            </div>
            <div style={{float:'right',marginRight:'20px'}}><h4 style={{color:'green'}}>Total Price: {tprice.toFixed(2)}</h4></div>    
